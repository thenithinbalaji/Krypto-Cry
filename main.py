...
try:
    ...
    @client.event
    async def on_ready():
        await client.change_presence(activity=discord.Activity(type=discord.ActivityType.listening, name="@mentions"))
    ...
    async def on_message(message):
        ...
        if client.user.mentioned_in(message):
            ...
            if len(word) == 0:
                await message.reply("Mention the crypto name along with the mention \nVisit https://bit.ly/kryptocryweb to know some valid crypto names!")
                ...
                if type(data) == list:
                    ...
                    embed.add_field(name = "Price", value= "$"+ str(round(float(usdprice), 3)), inline=True)

                    embed.add_field(name = "Market Cap", value= "$"+ str(round(float(marketcap), 3)), inline=True)
                    embed.add_field(name = "Change in 24h", value = str(round(float(change24h), 3)) + "%", inline=True)
                    embed.add_field(name = "Change in 7d", value = str(round(float(change7d), 3)) + "%", inline=True)
                    ...
                    embed = discord.Embed(title = "404 Error", color = 0xe74c3c, description = f"{formal_word} doesn't seem to be a valid crypto name")

                    embed.add_field(name = "List of Crypto Prices Available", value = "https://bit.ly/kryptocryweb")
